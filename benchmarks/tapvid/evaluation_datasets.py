...
def compute_tapvid_metrics(
    ...
    for thresh in [1, 2, 4, 8, 16]:
        ...
        false_negatives = gt_positives - true_positives
        metrics["false_negatives_" + str(thresh)] = int(false_negatives)
        metrics["false_positives_" + str(thresh)] = int(false_positives)
        metrics["true_positives_" + str(thresh)] = int(true_positives)
    ...
    metrics["false_positives"] = int(fps)
    metrics["false_negatives"] = int(fns)
    metrics["true_positives"] = int(tps)
...
def create_davis_dataset(
    ...
    if full_resolution:
        raise NotImplementedError("Full resolution not supported yet.")
        # ds, _ = tfds.load("davis/full_resolution", split="validation", with_info=True)
...
def create_kinetics_dataset(
    ...
    all_paths = sorted(glob.glob(path.join(kinetics_path, "*_of_0010.pkl")))
